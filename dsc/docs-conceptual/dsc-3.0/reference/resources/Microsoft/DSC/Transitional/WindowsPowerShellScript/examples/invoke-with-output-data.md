---
description: >
  Example showing how to return output data from a WindowsPowerShellScript
  resource.
ms.date:     05/10/2026
ms.topic:    reference
title:       Invoke the WindowsPowerShellScript resource with output data
---

<!-- markdownlint-disable MD025 -->

# Invoke the WindowsPowerShellScript resource with output data
...
$instance = @'
getScript: |-
   $filePath = 'Temp:/dsc/examples/WindowsPowerShellScript/output.txt'
...
```powershell
$fileInfo = Get-Item -Path 'Temp:/dsc/examples/WindowsPowerShellScript/output.txt'
...
script see [Invoke the WindowsPowerShellScript resource with trace messaging][06].
...
see [Invoke the WindowsPowerShellScript resource with trace messaging][06].
...
<timestamp>  INFO Invoking test on 'Microsoft.DSC.Transitional/WindowsPowerShellScript' using 'powershell'
<timestamp>  INFO PID <pid>: The file 'Temp:/dsc/examples/WindowsPowerShellScript/output.txt' is empty
...
[Invoke the WindowsPowerShellScript resource with trace messaging][06].
...
afterState:
  output:
  - File 'Temp:/dsc/examples/WindowsPowerShellScript/output.txt' is empty - adding conent
  - initialContent: null
...
$instance = [ordered]@{
    input = [ordered]@{
        filePath = 'Temp:/dsc/examples/WindowsPowerShellScript/output.json'
        settings = [ordered]@{
...
```powershell
dsc --trace-level info resource get --resource Microsoft.DSC.Transitional/WindowsPowerShellScript --input (
...
<timestamp>  INFO PID <pid>: Performing the operation "Create File" on target "Destination: C:\Users\<username>\AppData\Local\Temp\dsc\examples\WindowsPowerShellScript\output.json".
...
<timstamp>  INFO Getting current state for set by invoking get on 'Microsoft.DSC.Transitional/WindowsPowerShellScript' using 'powershell'
