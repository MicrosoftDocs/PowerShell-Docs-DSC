...
1. The fully qualified type name (`type` field) for the adapted instance is defined by the adapter.
   ...
   For example, if a PowerShell module named **TailspinToys** has a script-based PSDSC resource
   named `TSToy`, the fully qualified type name for that resource is `TailspinToys/TSToy`.
...
[04]: /powershell/module/microsoft.powershell.core/enable-psremoting?view=powershell-5.1&preserve-view=true
